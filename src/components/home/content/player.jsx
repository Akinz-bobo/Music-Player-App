...
import { FaRandom, FaPlay, FaPause, FaVolumeMute, FaVolumeUp, FaWindowClose } from "react-icons/fa"
import { BsFillSkipEndFill, BsSkipStartFill, BsHeart, BsHeartFill } from "react-icons/bs"
...
const Player = () => {
    ...
    function prev() {
        ...
        if (context.id === '') {
            ...
            context.songs[(idNum - 2).toString()].LikeMusic == "true" ? context.setLikedMusic(true) : context.setLikedMusic(false)
        }
    }
    ...
    function skip() {
        if (context.id === '') {
            ...
            context.songs[context.id].LikeMusic == "true" ? context.setLikedMusic(true) : context.setLikedMusic(false)
        }
    ...
    function randomMusic() {
        ...
        context.songs[(randomNum - 1).toString()].LikeMusic == "true" ? context.setLikedMusic(true) : context.setLikedMusic(false)
            ...
            <section className="player_container">
                ...
                <div className='controls'>
                    ...
                    <div className='button_controls'>
                        <button onClick={prev}><BsSkipStartFill /></button>
                        <button onClick={() => context.setPlaying(context.playing == true ? false : true)}>{context.playing == false ? <FaPlay /> : <FaPause />}</button>
                        <button onClick={skip}><BsFillSkipEndFill /></button>
                        <button className={random ? "activeRandom" : ""} onClick={randomMusic}><FaRandom /></button>
                        <button className={repeat ? "activeRandom" : ""} onClick={repeatOnce}>{repeat ? <TbRepeatOnce /> : <TbRepeatOff />}</button>
                    </div>
                ...
                {context.songs.map(s => (
                    ...
                    context.id === s.musicId ?
                        <div
                            className="containerDetailMusic"
                            key={s.musicId}>
                            <>
                                <button className={FullImg ? 'closeFullImg' : ""} onClick={() => setFullImg(false)}>{FullImg ? <FaWindowClose /> : ""}</button>
                                <div className={MusicPictureClass()}>

                                    <img src={s.cover} onClick={() => setFullImg(true)} />

                                    <div className={FullImg ? "showFullDetail_enable" : "showFullDetail_disable"}>
                                        <img src={s.cover} onClick={() => setFullImg(true)} />
                                        <span>{s.singer}</span>
                                ...
                                <span className='heart' onClick={() => context.likedMusic == false ? LikedMusic(s.musicId) : disLikeMusic(s.musicId)}>{context.likedMusic == true ? <BsHeartFill /> : <BsHeart />}</span>
